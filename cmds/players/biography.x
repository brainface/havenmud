...
mixed cmd(string args) {
    ...
    if(creatorp(this_player()) && args) {
       if(who = find_living(args)) {
          this_player()->eventPrint("%^BLUE%^Biography for " + capitalize(args));
       }
       else { return "Target " + args + " could not be found."; }
    }
    if(!who) who = this_player();

    cols = ((int *)this_player()->GetScreen())[0];
    tmp = ({ center("Biography for " + (string)who->GetShort() +
                    " on " + mud_name(), cols), "" });
    ...
    tmp += ({ sprintf("%:-"+(cols/2)+"s%"+(cols/2)+"s", "Level: " +
                      (int)who->GetLevel(), "Age: " +
                      ((time() - birth)/YEAR) + " years") });
    tmp = ({ "You are " + (string)who->GetMoralityDescription()
               + "." });
...
string GetHelp(string str) {
    string ret = "Syntax: biography\n";
    if(creatorp(this_player())) {
        ret+= "        biography <living>\n";
     }
    return (ret +=("\n"
