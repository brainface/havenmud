...
mixed cmd(string arg) {
    ...
    if( !arg ) arg = "index";
    if( arg == "index" || TALENT_HELP_D->GetTopics(arg) ) {
	if( arg == "index" ) {
	    HelpMenu();
	    return 1;
	}
    ...
    if( !(help = TALENT_HELP_D->GetHelp(arg)) ) {
	return TALENT_HELP_D->GetLastError();
    }
...
static int CanAccess(object who, string index) {
    return TALENT_HELP_D->CanAccess(who, index);
}
...
varargs void HelpMenu(string index) {
	...
	indices = filter(TALENT_HELP_D->GetIndices(),
    ...
    else if( !TALENT_HELP_D->CanAccess(this_player(), index) ) {
	...
	string array topics = TALENT_HELP_D->GetTopics(index);
	...
	input_to(function(string str, string ind) {
		...
		string array tmp2;
		if( !ind ) tmp2 = filter(TALENT_HELP_D->GetIndices(),
					 (: CanAccess(this_player(), $1) :));
		else tmp2 = TALENT_HELP_D->GetTopics(ind);    
		if( ind_num < 1 || ind_num > sizeof(tmp2) ) {
		    str = 0;
		    TALENT_HELP_D->SetError("Index number out of range.");
		}
		else str = tmp2[ind_num - 1];
	    }
	    if( !ind && !TALENT_HELP_D->GetTopics(str) ) {
		message("help", "Invalid index choice.", this_player());
		message("prompt", "Hit <return>: ", this_player());
	    ...
	    if( !(ret = TALENT_HELP_D->GetHelpByIndex(ind, str)) ) {
		message("help", TALENT_HELP_D->GetLastError(), this_player());
