...
mixed cmd(string args) {
    ...
    if (angelp(this_player()) && (ob != this_player())) return "No.";
...
string inventory(object ob, int level, int scan) {
    ...
    if( scan & OPT_I ) {
        string tmp;
        if( scan & OPT_F ) tmp = short_file(identify(ob)) + "\n" + ret + " ";
        else tmp = "";
    ...
    else ret += short_file(identify(ob));
    if( ( scan & OPT_D ) &&
...
string GetHelp(string str) {
    return "Syntax: <objects>\n"
           "        <objects [-e] [-d] [-i] [-f] [object]>\n\n"
           "Used alone this will return objects in your inventory. "
           "With an object (e.g. a player name) as an argument, this "
           "will scan the inventory of the object you name. "
           ...
           "See also: allstatus, cash, stat, skill, summ";
}
