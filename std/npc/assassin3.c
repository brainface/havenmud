/*
 * Third Assassin in the Beornwulf
 * bodyguard quest.
...
SetInventory( ([
 ...
 varargs int eventDie(object who) {
   object *questee = ({ });
   questee = filter(all_inventory(environment(who)), (: playerp($1) && ($1->GetProperty("LordBQuestStage4pt2") == 1) :));
   
   if (who->GetProperty("LordBQuestStage4pt2") == 1) {
      who->RemoveProperty("LordBQuestStage4pt2");
         who->SetProperty("LordBQuestStage5", 1);
        }
   questee->RemoveProperty("LordBQuestStage4pt2");
   questee->SetProperty("LordBQuestStage5", 1);
