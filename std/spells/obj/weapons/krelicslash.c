...
static void create() {
  item::create();
  SetKeyName("mithril blade");
  SetMaterial( ({ "mithril" }) );
  SetShort("a shimmering mithril blade");
  SetId( ({ "blade", "sword" }) );
  ...
  SetWeaponType("slash");
  SetDamageType(SLASH|SHOCK|MAGIC);
  SetLong(
    "The hilt of this sword is made from pure mithril while main portion of the blade is composed "
...
int eventStrike(object target) {
  ...
  switch (amt) {
    case 0..10:
      message("environment",
    "%^BOLD%^%^WHITE%^The blade seems to fade from existence for a momment.%^RESET%^",
        environment(target));
        amt += 20;
     break;
    case 11..15:
      message("environment",
      "%^BOLD%^%^BLUE%^The blade glows with light.%^RESET%^",
         environment(target));
        amt += 35;
      break;
    case 16..20:
      message("environment",
        "%^RED%^BOLD%^Electricity shoots out from the blade.%^RESET%^",
        environment(target));
        amt += 40;
      break;
    }
...
void DoFade(){
        ...
        environment(this_object())->eventPrint("%^BOLD%^%^RED%^The blade "
