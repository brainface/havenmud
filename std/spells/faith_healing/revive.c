...
static void create() {
  ...
  SetDifficulty(12);
  SetHelp( 
     "Allows the caster to channel physical energy into magical energy."
    );
  }
...
varargs int eventCast(object who, int level, mixed limb, object array tars) {
  ...
  if (who->GetMagicPoints () == who->GetMaxMagicPoints()) {
    send_messages("",
      "$agent_possessive_noun body rebels at having too much magic summoned into it. ",
  ...
  if (who->GetStaminaPoints () < 30) {
    send_messages("fail",
      "$agent_name $agent_verb in an attempt to revive $agent_reflexive.",
      who, 0, environment(who) );
     return 1;
   }
  who->AddMagicPoints(amt);
  send_messages("summon", 
    "$agent_name $agent_verb some energy into religious force.",
    who, 0, environment(who) );
  who->eventDisplayStatus();
  return 1;
  }
