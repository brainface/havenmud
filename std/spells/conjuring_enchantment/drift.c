//
// The 'dimensional rift' spell
// Sends someone into the fire planes
// Thoin@Haven
//
...
static void create() {
  ...
  SetHelp(
           ...
           "the dimensions, sending themself or their target "
           "into or out of the fire planes. This spell cannot be "
           "casted on those too young to survive the planes.");
}
...
int CanCast(object who, int level, string limb, object array targets) {
   ...
   if(!playerp(target)) {
      message("system","This spell may only be "
              "directed at a player.",who);
      return 0;
   }
...
int eventCast(object who, int level, mixed limb, object array targets) {
   object target = targets[0];
   int chance = level + random(who->GetSkillLevel("evokation"));
   chance += random(who->GetSkillLevel("enchantment"));
   ...
   if((chance <= 0) && (target != who)) {
      if(random(20) >= 10) {
         ...
         call_out(function(object guy) {
            ...
            if(sscanf(env, "/domains/planes/%s", blah)) {
               ...
               guy->eventMove("/domains/planes/virtual/fire/0,0");
            }
      ...
      send_messages("","A bright light flickers in the room for a split-"
                    "second before suddenly vanishing.", 
                     ({ environment(target) }));
      return 1;
   } else {
