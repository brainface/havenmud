#include <lib.h>
#include <magic.h>
inherit LIB_SPELL;


static void create() {
  spell::create();
  SetSpell("elven grace");
  SetRules("OBJ", "LIV");
  SetSpellType(SPELL_OTHER);
  SetMagicCost(20, 20);
  SetReligions( ({ "Kylin" }) );
  SetSkills( ([
    "natural magic" : 10,
    ]) );
  SetHelp(
    "Blesses an object or person, causing it to perform better in combat. \n"
    "This version of the bless spell is exclusive to Kylin druids."
    );
}

varargs int eventCast(object who, int level, mixed blah, object *targets) {
  int amt, time;
  object target = targets[0];
  amt = who->GetSkillLevel("natural magic") * level/100;
  if (amt < 1) amt = 1;
  if (amt > 20) amt = 20;
  if (!living(target)) {
    time = level + (who->GetSkillLevel("natural magic"));
    send_messages("grace",
     "$agent_name%^BOLD%^GREEN%^ $agent_verb%^RESET%^ $target_name.",
      who, target, environment(who));
      target->eventBless(amt, time);
    return 1;
  }
  if (living(target)) {
    send_messages("grace",
     "$agent_name%^BOLD%^GREEN%^ $agent_verb%^RESET%^ $target_name.",
      who, target, environment(who));
    target->AddBlessed(amt);
    return 1;
  }
  return 1;
}   
