...
static void create() {
   ...
   SetMessages( ({
    ({ "irritate", "$agent_name %^BOLD%^WHITE%^$agent_verb%^RESET%^ $target_name with a %^BOLD%^BLACK%^small cloud of smoke%^RESET%^ from $agent_possessive open palm." }),
    ({ "scald", "$agent_name%^BOLD%^RED%^ $agent_verb%^RESET%^ $target_name with a %^BOLD%^BLACK%^plume of smoke%^RESET%^ from $agent_possessive open palm." }),
    ({ "scour", "$agent_name%^RED%^ $agent_verb%^RESET%^ $target_name with a %^BOLD%^BLACK%^plume of smoke and ash%^RESET%^ from $agent_possessive open palm." }),
    }) );
}
