...
mixed do_marry_liv_to_liv(object spouse1, object spouse2) {
 ...
 // }
  ...
  ring = new(STD_OBJ "wed_ring");
        ...
        send_messages( ({ "join" }),
          "$agent_name $agent_verb " + capitalize(spouse1->GetKeyName()) + " and " +
capitalize(spouse2->GetKeyName())
          + " in marriage.", this_player(), 0, environment(this_player()) );
        CHAT_D->eventSendChannel("Wedding!", "ooc", capitalize(spouse1->GetKeyName()) + " and " +
capitalize(spouse2->GetKeyName())
