...
mixed can_shiv() {
  if (this_player()->GetInCombat()) { return "shiv who?"; }
...
mixed do_shiv() {
  return do_shiv_liv(this_player()->GetCurrentEnemy());
}
...
int eventAttack(object who, object target) {
  ...
  if (result == -1) {
    send_messages( ({ "swing" }), "$agent_name %^BOLD%^ $agent_verb "
      "%^RESET%^ widly at the thin air $target_name occupied moments before!",
      who, target, environment(who));
    who->eventCollapse();
    return 1;
  }
  ...
  send_messages("stab",
    ...
    " $target_possessive_noun " + limb + " violently with $agent_possessive "
    + weapon->GetKeyName() + "!",
    who, target, environment(who) );
