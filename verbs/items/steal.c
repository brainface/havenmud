...
mixed can_steal_wrd_from_liv(string wrd) {
   ...
   if (this_player()->GetPosition() != POSITION_STANDING) 
     return "You should try that standing up!";
...
mixed do_steal_obj_from_liv(object item, object liv) {
    if (!item) {
      this_player()->eventPrint("But that isn't there!");
      return 1;
      }
    if( environment(item) != liv ) {
    this_player()->eventPrint((string)liv->GetName() + " does not seem to have that.");
        return 1;
    }
    this_player()->eventPrint("You eye " + (string)liv->GetName() +
			      " with thoughts on " + possessive(liv) + " " +
