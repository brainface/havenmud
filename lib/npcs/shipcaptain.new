...
// mahkiefel again:
//   shipcaptains no longer board in the coast. This is to keep ferry captains from boarding random damned player ships
//   and getting stuck.
//  (ports are coastal. You don't drown in the coast.)
//   ferrycaptains and general guys like that need a line similiar to :
//    SetAction(5, "board first ferry") so they'll board their ferries at coast if something goes wrong.
...
void heart_beat() {
  ...
  if (!environment(ship)) {
    if (!ship->GetProperty("coastal")) {
    com = "board ship";
  } // prevent ferry captains from boarding whatever the fuck they want
  }
  ...
  if (!vehiclep(ship) && !ship->GetProperty("coastal")) { com = "board 1st ship" ; }
