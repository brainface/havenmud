...
void eventPreview(object who, string args) {
    ...
    if (GetBoostSkill(GetTown(), who) && CanBoost()) tmp += ({ GetBoostSkill(GetTown(), who) + " (secondary)" });
    if (sizeof(tmp))
  ...
  if( args != GetClass() && args[0..<2] != GetClass() && args != pluralize(GetClass()) )
{
  ...
  if (who->GetClass()) {
    if (who->GetDevelopmentPoints() < 50) {
    	eventForce("speak You are not prepared to multiclass.");
      who->eventPrint("You must have 50 development points for that.");
      return;
    }
  }
  ...
  if (who->GetClass() != GetClass()) who->AddDevelopmentPoints(-50);
  ...
  if ((tmp = GetBoostSkill(GetTown(), who)) && CanBoost()) {
