...
object liv;
  ...
  if(liv = find_living(args))
  {
    ...
    xp    = liv->GetExperience();
    return this_player()->eventPrint( hpx+hp+s+maxhp+mpx+mp+s+maxmp+spx+sp+s+maxsp+b+xpx+xp+xpe );
  }
