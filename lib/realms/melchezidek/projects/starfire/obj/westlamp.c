...
varargs mixed eventLight(object who, object tool) {
    SetLit(1);
    SF_ROOM "room06"->SetProperty("westlamplit", 1);
    send_messages("mymessage", "A low rumble can be heard to the east.", who, 0, environment(who));
    SetLong("This large pillar has been carved from solid obsidian."
            " It has been crafted to resemble a burning flame. At"
            " the top of the pillar sits a basin with oil that shows"
            " signs of recent use."
           );
    return 1;
}
...
mixed eventDarken() {
  ...
  if( env = environment() ) {
   ...
   send_messages("mymessage2", "A loud scream erupts from the east.", who, 0, environment(who));
   SetLit(0);
   return 1;
}
