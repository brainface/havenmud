...
void eventPerform(int i) {
  ...
  switch(i) {
    ...
    case 3:
      send_messages( ({ }),
        "Aria slowly runs her hands down, moving around to face $target_name directly. Kneelingdown, "
    ...
    case 4:
      ...
      if (who->GetGender() == "male") {
        ...
        if (who->GetSize() < GetSize()) {
          who->eventPrint("If Jennifer is not impressed with your " + who->GetRace() + " endowments, she does "
      ...
      case 5:
        if (who->GetGender() == "male") {
          ...
          send_messages( ({ "arch" }),
            "jennifer moves her head and tongue faster and harder. Her hands move to $target_possessive_noun "
  ...
  CHAT_D->eventSendChannel("Jennifer", "reports", capitalize(who->GetKeyName()) + " paid jennifer for luvvin.");
