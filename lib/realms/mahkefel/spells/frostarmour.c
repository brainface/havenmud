...
int CreateFrostArmour(object who, object target, int AC, int armour_type, string quality, string thickness, string id, int
durability,
string limb );
...
varargs int eventCast(object who, int level, mixed n, object *t) {
  ...
  who->SetProperty("channeling",1);

  foreach (string limb in target->GetLimbs()) {
    exhaust = 1;
    if (CanWearArmour(target, limb, A_HELMET)) {
      exhaust++;
      call_out( (: CreateFrostArmour, who, target, AC, A_HELMET, quality, thickness, "helm", durability, limb :), exhaust );
    } else if (CanWearArmour(target, limb, A_ARMOUR)) {
      exhaust++;
      call_out( (: CreateFrostArmour, who, target, AC, A_ARMOUR, quality, thickness, "cuirass", durability, limb :), exhaust );
    } else if (CanWearArmour(target, limb, A_GLOVE)) {
      exhaust++;
      call_out( (: CreateFrostArmour, who, target, AC, A_GLOVE, quality, thickness, "gauntlet", durability, limb :), exhaust );
    } else if (CanWearArmour(target, limb, A_PANTS)) {
      exhaust++;
      call_out( (: CreateFrostArmour, who, target, AC, A_PANTS, quality, thickness, "greave", durability, limb :), exhaust);
    } else if (CanWearArmour(target, limb, A_BOOT)) {
      exhaust++;
      call_out( (: CreateFrostArmour, who, target, AC, A_BOOT, quality, thickness, "boot", durability, limb :), exhaust);
    } else if (CanWearArmour(target, limb, A_WING)) {
      exhaust++;
      call_out( (: CreateFrostArmour, who, target, AC, A_WING, quality, thickness, "wing guard", durability, limb :), exhaust );
    } else if (CanWearArmour(target, limb, A_TAIL)) {
      exhaust++;
      call_out( (: CreateFrostArmour, who, target, AC, A_TAIL, quality, thickness, "tail", durability, limb :), exhaust );
    }

    
    if (exhaust) {
      who->AddCondition("Channeling Create Frost Armour", CONDITION_PREVENT_MOVE|CONDITION_PREVENT_MAGIC|CONDITION_PREVENT_COMBAT, exhaust);
...
string limb ) {
  ...
  if (armour->eventMove(target)) {
    ...
    who->RemoveProperty("Channeling Create Frost Armour");
    return 0;
  }
