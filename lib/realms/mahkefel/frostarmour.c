...
varargs int eventCast(object who, int level, mixed n, object *t) {
  ...
  limbs = target->GetLimbs();
  foreach (string limb in limbs) {
    exhaust = 0;
    if (CanWearArmour(target, limb, A_HELMET)) {
      exhaust++;
      CreateFrostArmour(target, AC, A_HELMET, quality, thickness, "helm", durability, limb );        
    } else if (CanWearArmour(target, limb, A_ARMOUR)) {
      exhaust++;
      CreateFrostArmour(target, AC, A_ARMOUR, quality, thickness, "cuirass", durability, limb );        
    } else if (CanWearArmour(target, limb, A_GLOVE)) {
      exhaust++;
      CreateFrostArmour(target, AC, A_GLOVE, quality, thickness, "gauntlet", durability, limb );        
    } else if (CanWearArmour(target, limb, A_PANTS)) {
      exhaust++;
      CreateFrostArmour(target, AC, A_PANTS, quality, thickness, "greave", durability, limb );        
    } else if (CanWearArmour(target, limb, A_BOOT)) {
      exhaust++;
      CreateFrostArmour(target, AC, A_BOOT, quality, thickness, "boot", durability, limb );        
    } else if (CanWearArmour(target, limb, A_WING)) {
      exhaust++;
      CreateFrostArmour(target, AC, A_WING, quality, thickness, "wing guard", durability, limb );        
    } else if (CanWearArmour(target, limb, A_TAIL)) {
      exhaust++;
      CreateFrostArmour(target, AC, A_TAIL, quality, thickness, "tail", durability, limb );        
    }
  
    if (exhaust) {
      // drain some stamina
      //who->AddStaminaPoints(-100);
      if (who->GetStaminaPoints() < 10) {
...
int CreateFrostArmour(object target, int AC, int armour_type, string quality, string thickness, string id, int durability, string limb ) {
  ...
  if (target->CanWear(armour, limb)) {
    armour->eventMove(target);

    target->eventWear(armour, limb);
    debug(identify(environment(armour)));
    } else {
      debug("failed to CanWear");
      armour->eventDestruct();
    }
    debug("CanWear success - checking for Worn");
