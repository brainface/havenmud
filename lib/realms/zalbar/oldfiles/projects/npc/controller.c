...
static void create() {
  ...
  SetReligion("Kylin");
  SetLong(
    "Gideon is an Inquisitor of Kylin. Grand Master Banewulf has assign him to "
    "coordinate the attacks on the daemons remaining in the ruins of Durgoroth. "
    "If you are interested in helping the Inquisitors, <request quest from "
    "gideon> or <ask gideon for quest>." );
  SetMorality(2000);
  SetInventory( ([
    ]) );
...
void DoSomething() {
	switch(10) {
  	  ...
  	  break;
  	case 2:
  		eventForce("speak the time is coming near to strike.");
  		break;
  	case 3:
  		eventForce("yell We must prepare for the assault.");
  		break;
  	deafult:
  		eventForce("speak Have you come here to join the assault?");
  		break;	
  }
}
