...
void eventStomp() {
  ...
  if (num < 6){
	  if(present(target)) {
		...
		"creating a shockwave $agent_verb $target_name knocking $target_objective "
		"down!%^RESET%^",this_object(), target, environment(target) );
		target->eventCollapse(1);
		target->SetParalyzed(5);
		target->eventReceiveDamage(this_object(),MAGIC,1500);
		}
  }
  else {
	  if(present(target)){
		...
		target->eventReceiveDamage(this_object(),POISON,1500);
		}
  }
  return;
}
