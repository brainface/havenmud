...
mixed cmd(string args) {
    ...
    if (angelp(this_player())) return "You don't need to know that.";
    ...
    lines += ({ center("Food: " + (int)ob->GetFood() + "  " +
                       ...
                       "Smoke: " + (int)ob->GetSmoke() + "  " +
                       "Poison: " + (int)ob->GetPoison() + "  " +
                       "Daze: " + (int)ob->GetRecoveryTime(), cols) });
    lines += ({ center("Development Points: " + (int)ob->GetDevelopmentPoints())   });
    lines += ({ "", "Limbs:" });
