...
static void heart_beat() {
	...
	if (!who) {
	  ...
	  foreach(object ship in filter(all_inventory(), (: vehiclep :) )) {	  
	    if (who = present(fisher, ship)) break;
	  }
	}
	...
	if( !Chance || !(x = (int)pole->eventFish(who)) ) chance = 0;
	    ...
	    chance = (Chance + x + (int)who->GetStatLevel("luck")) / (1 + random(5));
	}
...
static void eventCatch(object who, string fish, object pole) {
    ...
    who->AddExperience((food->GetMass() * who->GetLevel()) + (who->GetSkillLevel("fishing")/5 ));
	...
	message("other_action", (string)who->GetName() + " drops " +
		(string)food->GetShort() + "!", environment(who), ({ who }) );
