...
static void create()
{
  ...
  if( !Classes ) Classes = ([]);
  if( !MudSkills ) MudSkills = ({ });

   foreach( string classname in GetClasses() ) {
   	class Class cls;
   	cls = new(class Class);
   	cls = Classes[classname];
    foreach( string skill in keys(cls->Skills) ) {
      if( member_array(skill, MudSkills) == -1 ) {
      	 MudSkills += ({ skill }); 
      }
    }
  }
...
string GetHelp(string class_name) {
    ...
    if( !sizeof(cls->Multis) ) {
      help += capitalize(pluralize(class_name)) + " cannot multi-class.\n";
    } else {
	      help += capitalize(pluralize(class_name)) + " can multi-class with the following primary classes:\n";
	      foreach(string prime, string other in cls->Multis) {
	        help += "\t" + capitalize(class_name) + " + " + prime + " = " + other +  "\n";
	      }
    }
    help += "\n\n" + capitalize(pluralize(class_name)) + " have the following primary skills:\n";
    foreach(string skill, class Skill s in cls->Skills) {
	    if( s->SkillClass == 1 ) { h += ({ skill  }); }
	  }
	  help += "\t" + implode(sort_array(h, 1), "\n\t");
	  h =({ });
    help += "\n\n" + capitalize(pluralize(class_name)) + " have the following secondary skills:\n";
    foreach(string skill, class Skill s in cls->Skills) {
	    if( s->SkillClass == 2 ) { h += ({ skill });  }
	  }  	
	  help += "\t" + implode(sort_array(h, 1), "\n\t");
	  h =({ });
    help += "\n\n" + capitalize(pluralize(class_name)) + " have the following other skills:\n";
    foreach(string skill, class Skill s in cls->Skills) {
      if( s->SkillClass > 2 ) { h += ({ skill  }); }
	  }
	  help += "\t" + implode(sort_array(h, 1), "\n\t");
...
int GetValidSkill(string skill) {
  if( member_array(skill, MudSkills) != -1 )
    return 1;
  else return 0;
}

string *GetBaseClasses(string multi) {
  string *ret = ({});

  foreach(string p_class in GetClasses()) {
    if (ClassMember(multi, p_class)) ret += ({ p_class });
    }
  return ret;
}
