...
static void create()
{
  ...
  npc::create();
  SetKeyName("test squid");
  SetId(({"squid","giant squid","monster"}) );
  SetShort("squishy the testing squid");
  SetLong(
    "Tentacles wave everywhere from the leviathan before you as "
...
*/
  ...
  SetCombatAction(100, (: AttemptGrapple :) );
...
debug("Attempting grapple.");
  if(living(target = all_inventory(environment(this_player()))[i]) &&
     target != this_player()  && !creatorp(target))
  {
    if(1 == 1) {
/*
    if(random(100) < (15 + (
    ...
    {
*/
debug("Going to DoGrapple.");
      DoGrapple(target,this_tentacle);
    } else {
debug("Failed Grapple attempt.");
      message("environment",
          "%^BOLD%^%^YELLOW%^"
...
void DoGrapple(object target,int which_tentacle)
{
// No paralysis check needed here because it's a combat action so
// won't happen if squid is paralyzed.
...
debug("Calling out DoConstrict.");
  call_out( (: DoConstrict :), 2, target, which_tentacle);
}
...
// Paralysis check added by Laoise, April 2005
  if(this_object()->GetParalyzed()) {
debug("Doing paralysis check for Do Constrict");
  message("environment",
    "%^BOLD%^%^YELLOW%^"
    "Stunned by your fierce attacks, the squid allows you to "
    "slip from its grasp.%^RESET%^",
    target);
  message("environment",
    "%^BOLD%^%^YELLOW%^" +
    target->GetName() + " manages to slip from the squid's grasp "
    "as it lies stunned.%^RESET%^",
    environment(this_player()),target);
  return;
  }

  if(GetLimb(tentacle_name)) {
debug("Actually Constricting");
/*
    target->eventReceiveDamage(0,0,random(25)+10);
*/
debug("Damaging "+target->GetKeyName());
    message("environment",
            "%^BOLD%^%^YELLOW%^"
...
debug("Calling Do Constrict out again.");
  }
  else
  {
debug("Severing tenticle.");
