/*
 * Fourth Assassin in the Beornwulf
 * bodyguard quest.
...
SetInventory( ([
 ...
 varargs int eventDie(object who) {
   object *questee = ({ });
   questee = filter(all_inventory(environment(who)), (: playerp($1) && ($1->GetProperty("LordBQuestStage6pt1") == 1) :));
   
   if (who->GetProperty("LordBQuestStage6pt1") == 1) {
      who->RemoveProperty("LordBQuestStage6pt1");
         who->SetProperty("LordBQuestStage6pt2", 1);
        }
   questee->RemoveProperty("LordBQuestStage6pt1");
   questee->SetProperty("LordBQuestStage6pt2", 1);
