...
int eventStyle(object who, int level, mixed target) {
  ...
  attack += who->GetSkillLevel("slash combat");

  defense = target->GetDefense();
  defense -= who->GetDefense()/4;
  ...
  dmg = who->GetSkillLevel("slash combat");
  def = who->GetDefense();
  ...
  if (dmg <= 25) {
      ...
      switch (random(12)) {
        ...
        case 1:
           ...
           target->eventReceiveDamage(who, SLASH, 
                 who->GetDamage(who->GetWielded()[0]->eventStrike(target), "slash combat", target->GetDefense()) + dmg/6, 0, tmp);
	         break;
	   	  default:
	   	     break;
        }
        ...
        case 1:
           ...
           target->eventReceiveDamage(who, SLASH, 
               who->GetDamage(who->GetWielded()[0]->eventStrike(target),         
		           "slash combat", target->GetDefense()) + dmg/6, 0, tmp);
	         break;	   
        case 2:
	         send_messages( ({ "slice" }),
	         ...
	         target->eventReceiveDamage(who, SLASH,
		         who->GetDamage(who->GetWielded()[0]->eventStrike(target),
			       "slash combat", target->GetDefense()) + dmg/4, 0, tmp);
	         break;
    		default:
     		  break;
      }
	...
	case 0:
	   ...
	   "$agent_possessive weapon deeply into $target_possessive chest!",
	   who, target, environment(who));
	   target->eventReceiveDamage(who, PIERCE,
		who->GetDamage(who->GetWielded()[0]->eventStrike(target),
			"slash combat", target->GetDefense()) + dmg/2, 0, tmp);
	   break;
    		  default:
     		  break;
      }
	...
	case 4:
	   ...
	   target->eventReceiveDamage(who, SLASH,
	   	who->GetDamage(who->GetWielded()[0]->eventStrike(target),
			"slash combat", target->GetDefense()) + dmg, 0, tmp);
    		  default:
     		  break;
      }
	return 1;
	...
	case 4:
	   ...
	   target->eventReceiveDamage(who, SLASH,
	   	who->GetDamage(who->GetWielded()[0]->eventStrike(target),
			"slash combat", target->GetDefense()) + dmg+def/10, 0, tmp);
	        who->eventWeaponRound(target, who->GetWielded());
    		  default:
     		  break;
      }
