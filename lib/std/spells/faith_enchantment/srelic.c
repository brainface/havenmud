...
static void create() {
  ...
  SetHelp(
          "This spell allows a Cleric to summon forth a divine weapon of their "
...
object who = this_player();
  int time, dmg, level;
  level = who->GetSpellLevel("summon relic");
  time = (level * 15 + 10 + who->GetSkillLevel("enchantment")/20);
