...
static void create() {
  spell::create();
  SetSpell("superior wall of force");
  SetRules("", "LIV");
  SetDamagingShield(1);
  SetReligions("Kylin");
  ...
  SetMessages( ({ 
  	({ "appear",
       "$target_name $target_verb %^BLUE%^supremely protected%^RESET%^ by the "
...
int hitCallback(object who, object agent, int x, class MagicProtection cl) {
   ...
   send_messages("", 
      "The " + colour + "superior wall of force %^RESET%^protecting "
...
/*  if (agent && living(agent) )
  ...
  agent->eventReceiveDamage(who, MAGIC, who->GetSpellLevel("superior wall of force"), 0, agent->GetRandomLimb()); */
    return x;
}
...
void endCallback(object who) {
    if( who ) {
   send_messages("", 
      "The %^BLUE%^superior wall of force%^RESET%^ around $agent_name vanishes.",
