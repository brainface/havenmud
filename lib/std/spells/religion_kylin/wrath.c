...
static void create() {
  ...
  SetHelp(
          "This spell showers a target with Duuktsaryth's divine wrath."
          );
  SetMessages( ({
    ...
    ({ "unleash","$agent_name $agent_verb%^YELLOW%^ the fiery vengeance of Duuktsaryth %^RESET%^on $target_name.", })
    }) );
}
