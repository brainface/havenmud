...
static void create() {
  ...
  SetSpell("advanced chain lightning");
  SetDifficulty(40);
  SetSkills( ([
     "conjuring" : 275,
     "evokation" : 275,
  ...
  SetHelp(
    ...
    "into the target. It is a more powerful and more deadly spell.");
}
...
varargs int eventCast(object who, int level, mixed limb, object array targets) { 
  ...
  send_messages( ({ "unleash", }),
    "$agent_name $agent_verb a massive rapid-fire hail of lightning on $target_name!",
  ...
  while(i < num_hits) {
    ...
    if (present(victim, environment(who)) && living(victim)) {
      send_messages("sear",
         ...
         r_limb + " with a massive bolt of energy.",
         who, victim, environment(who));
