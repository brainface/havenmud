...
static void create() {
 ...
 //
   SetMessages( ({
     ({ "fail", "$agent_name $agent_verb to maintain a chanting "
                "prayer." }),
	 ({ "maintain", "$agent_name barely $agent_verb the chant "
                    "before a %^RED%^flaming arrow%^RESET%^ appears and hurls "
                    "itself at $target_name.%^RESET%^" }),
	 ({ "summon", "Through the power of a soft chant $agent_name "
                 "$agent_verb %^RED%^BOLD%^a flaming arrow%^RESET%^ which "
