...
static void create() {
        ...
        AddExit("north",D_ROOM+"/road03",(:Ranllfixit:));
...
int Ranllfixit() {
  object sentinel = present("sentinel", this_object());
  object who = this_player();
  if (!sentinel) return 1;
  if (who->GetInvis() ) return 1;
  if (who->GetTown() != "Haven" && who->GetReligion() != "Kylin") return 1;
  sentinel->eventForce("say We don't allow you scum to pollute our fair city");
     message("system", who->GetName() + " tries to go north.", this_object() );
  return 0;
 }
