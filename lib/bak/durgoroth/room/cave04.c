...
static void create() {
	...
	SetItems( ([
		...
		({"massive edifice","edifice","portal"}) : "The massive "
			...
			"portal the tapestry of Arika can be seen on the far "
			"wall.",
		...
		({"large tapestry","tapestry"}) : "The large tapestry on the "
			...
			"a mound of the skulls of her slain enemies, while in the "
	...
	SetExits( ([
        ...
        AddEnter("portal", DOMAIN_PLANES "virtual/fire/0,0",(:Womble:));
}
  ...
  if (who->GetLevel() < 50) {
    who->eventPrint("You cannot enter that yet.");
    return 0;
  }
  return 1;
}
