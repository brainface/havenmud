...
inherit LIB_ROOM;
#include "balz.h"
#define EXIT "/domains/westwood/virtual/soleilroad/0,5"
...
static void create() {
   ...
   SetLong("One tall tree stands in the middle of the path here, almost "
           ...
           "gaps in the thick trees appear to the north, northwest, "
           "and northeast.  The forest opens up to the south.");
   SetItems( ([
                ...
                ({ "small gaps","gaps","gap","small gap" }) :
                   ...
                   "pathway to walk through to the northeast, north, "
                   "and northwest.",
   ...
   SetExits( ([
                "north" : B_ROOM + "/ete",
                "northwest" : B_ROOM + "/etb",
                "northeast" : B_ROOM + "/etc",
           ]) );
   AddExit("south",EXIT,(: PreExit :));
   SetObviousExits("south, north, northwest, northeast");
   SetInventory( ([
...
void TreeWink() {
   message("other_action","One of the knots in the tall tree "
      ...
      "taking in its surroundings and the presence of life.",
      this_object());
...
int PreExit() {
   if(this_player()->GetProperty("enchforestnoleave")) return 0;
   return 1;
}
