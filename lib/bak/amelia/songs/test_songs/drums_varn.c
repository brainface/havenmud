...
static void create() {
  ...
  SetHelp("Syntax: <sing drums of karak varn>\n\n"
    ...
    "the Drums of Karak Varn has been known to enspire the "
...
int eventSing(object who, int level, mixed limb, object *targets) {
  ...
  bonus = ( (who->GetLevel() + who->GetSpellLevel("drums of karak varn") )/3 );
  songlength = 
    ((who->GetLevel()) + who->
  ...
  if(chance >= 16) {
    send_messages("sing",  "As $agent_name $agent_verb the deep and "
                  "harmonic "
                  "%^MAGENTA%^%^BOLD%^Drums of Karak Varn%^RESET%^"
                  ", everyone "
...
void eventFeelThePower() {
  object *who;
  who = filter(users(), (: noncreatorp :) );
  foreach(object user in who)
  {
  if( (user->GetProperty("drumsvarn", 1) ) ) {
    user->RemoveProperty("drumsvarn");
    user->RemoveStatBonus("strength");
    user->RemoveStatBonus("durability");
    send_messages("", "With the final stanza of "
                      "the Drums of Karak Varn "
                      "coming to an end, everyone "
                      "feals a sense of pride and at ease."
                      ,user,0,environment(user));
    }
  }
  }
