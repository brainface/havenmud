...
private int HealthPoints, MagicPoints, Position;
private int Alcohol, Caffeine, Food, Drink, Poison, Sleeping, Blind;
private float StaminaPoints;
private string Torso;
private mapping Fingers, Limbs, MissingLimbs;
private static int Dying, LastHeal;
private static int LastMessages;
...
static void heart_beat() {
	...
	if( (i = env->GetPoisonGas()) > 0 ) {
	    eventPrint("You choke on the poisonous gases.");
            ...
            eventReceiveDamage(env, POISON, i, 1);
	}
    }
    eventCheckHealing();
}
...
int eventCollapse() {
    SetParalyzed(2, (: checkCollapse :));
    if( Position == POSITION_LYING ) {
	return 1;
    }
  message("system",
        "%^RED%^BOLD%^You collapse to the ground.%^RESET%^",
          this_object() );
  message("system",
        "%^RED%^BOLD%^" + GetName() + " collapses to the ground.%^RESET%^",
...
void eventCheckHealing()
{
  ...
  if( (y = time() - LastHeal)  >= x )
  {
    ...
    if (!GetUndead())
    {
      if( Alcohol > 0 )
      {
        ...
        else if( !GetSleeping() && random(100) < 8 )
        {
          ...
          send_messages( verb, "$agent_name $agent_verb " + adv + ".",
    ...
    if( Sleeping > 0 )
    {
      Sleeping--;
      if( !Sleeping )
        send_messages( "wake", "$agent_name $agent_verb from " +
                       "$agent_possessive deep sleep.", this_object(), 0,
                       environment());
      else if( random(100) < 8 )
      send_messages("snore",
        "$agent_name $agent_verb " +
        (GetGender() == "male" ? "loudly" : "softly") + 
        ".",
                       this_object(), 0, environment());
    }
    if( Blind > 0 )
    {
      ...
      if( !Blind )
      {
        message("my_action", "You can see again!", this_object());
        message("other_action", GetName() + " rubs " +
          possessive(this_object()) + " eyes and blinks.",
    ...
    if (GetHiding()) {
    AddStaminaPoints(-20);
      if (GetStaminaPoints() < 20) {
        SetHiding(0);
        }
      }
    if (GetRecoveryTime()) { 
        AddRecoveryTime(-10);
         if (GetRecoveryTime() < 1) {
              SetRecoveryTime(0);
             }
         if (!GetRecoveryTime())
           message("system", "You are able to focus your mind again.",
                             this_object() );
       }
    if( playerp(this_object()) )
    {
      ...
      if( Drink > 0) Drink--;
     if (!Food || !Drink) {
        AddHealthPoints(-GetHealRate());
       }
    }
    if( Drink < 0) Drink = 0;
    if( Poison > 0) Poison--;
    if( this_object()->GetInCombat() && 
      !(this_object()->GetPosition() == POSITION_LYING) )
      AddStaminaPoints(-(random(6)+15) );
    eventCheckStatus();
  }
}
...
mixed eventFall() {
    ...
    dest = env->GetGround();
    if( !dest ) {
	eventPrint("You fall into a dark abyss and die.");
	if( environment() ) {
	    environment()->eventPrint(GetName() + " falls into the dark "
				      "abyss and dies.", this_object());
	}
	Position = POSITION_LYING;
	eventDie();
    }
	...
	eventPrint("You fall through the sky towards the world below.");
	environment()->eventPrint(GetName() + " falls through the sky to the "
				  "world below.", this_object());
	eventMove(dest);
...
mixed limbs) {
    ...
    float modifier=1.0;
    int onesdone=0;
    while((1 << scanner) <= MAX_DAMAGE_BIT)
    {
      if((1 << scanner) & type)
        {
          string tmp = GetResistance(1 << scanner);            
          switch(tmp) 
          {
            case "immune": modifier=(onesdone*modifier+0)/(onesdone+1); break;
            case "low": modifier=(onesdone*modifier+0.75)/(onesdone+1); break;
            case "medium": modifier=(onesdone*modifier+0.5)/(onesdone+1); break;
            case "high": modifier=(onesdone*modifier+0.25)/(onesdone+1); break;
            case "weakness" : modifier=(onesdone*modifier+2)/(onesdone+1); break;
            case "none" : modifier=(onesdone*modifier+1)/(onesdone+1); break;
          }      
          onesdone++;
        }
        scanner++;
...
int eventCheckProtection(object agent, int type, int damage) {
    ...
    while(i--) {
        ...
        if( !(type & Protection[i]->bits) ) continue;
         while((1 << a) <= MAX_DAMAGE_BIT) {
          if ((type & (1 << a)) && (Protection[i]->bits & (1 << a))) blocked++;
          if ((type & (1 << a)) && !(Protection[i]->bits & (1 << a))) nonblocked++;
          a++;
            }
        if( Protection[i]->absorb ) {
            if(blocked+nonblocked==0)
            {
              this_player()->eventPrint("This is a bug.");
              blocked=1;
            }
            if( (x = (Protection[i]->absorb - 
            to_int(damage * (blocked/(blocked + nonblocked + 0.0))) ) ) < 1) {
            ...
            Protection[i]->absorb -= to_int(damage * 
                        (blocked/(blocked + nonblocked + 0.0)));
        }
        else if( Protection[i]->protect )
            x = (Protection[i]->protect * to_int(blocked/(blocked + 
                        nonblocked + 0.0)));
        else {
            RemoveMagicProtection(i);
            continue;
        ...
        damage -= to_int(random(to_int(4*x/5)) + (x/5));
        if( damage < 1 ) return 0;
    }
    return damage;
}
 ...
 */
  ...
  foreach(object thing in WornItems[limb]) {
     if (sizeof(thing->GetWorn()) < 2) {
         thing->SetWorn(0);
         thing->eventMove(ob);
         eventRemoveItem(thing);
        } else {
              thing->eventUnequip(this_object());
              thing->eventEquipAgain();
           }
     }
    }
    return 1;
...
varargs object *GetMetalWorn(string irrelevant) {
  object *metalworn = ({});

  foreach(string limb in GetLimbs()) {
    foreach(object worn in GetWorn(limb)) {
      if (member_array("metal", worn->GetMaterials()) != -1 && 
           !(worn->GetArmourType() & (A_WEAPON|A_RING|A_AMULET)) )
        metalworn += ({ worn });
      }
    }
  return metalworn;
...
int AddMagicProtection(class MagicProtection cl) {
   ...
   foreach(class MagicProtection pro in GetMagicProtection()) {
      if(pro->name==cl->name)
        count++;
      if(count>=(cl->limit))
        return 0;
...
void eventCheckStatus() {
  ...
  switch ( (GetFood()/8) ) {
     case 0:
       eventReceiveDamage(this_object(),HUNGER,random(8),1);
  ...
  switch ( (GetDrink()/8) ) {
    case 0:
        ...
        eventReceiveDamage(this_object(), HUNGER, random(3), 1);
        break;
    case 1:
  ...
  if (GetPoison() > 0) {
     ...
     eventReceiveDamage(this_object(), POISON, random(GetPoison()), 1);
    }
  }
