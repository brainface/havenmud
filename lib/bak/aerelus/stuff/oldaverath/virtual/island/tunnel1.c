...
#include <lib.h>
#include <domains.h>
#define MOUN_EXIT   AVERATH_ISLAND "-6,-3,0"
#include "../glim.h"
...
static void create() {
  ...
  SetListen( ([
    "default" : "Movement some distance away occasionally breaks the
silence.",
  ]) );
   AddExit("out",MOUN_EXIT, (: PreExit :));
}
int PreExit(string dir) {
   if(this_player()->id("glim_wander")) { return 0; }
   else return 1;
}
