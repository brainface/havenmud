...
int eventTend(object tender, object tendee) {
        int amount = tender->GetSkillLevel("healing")/5 + 1;
	string msg;
	...
	tender->eventTrainSkill("healing", amount, 100, 1);
...
void eventCheckHealing() {
    ...
    if( (y = time() - LastHeal)  >= x ) {
        ...
        do {
          ...
          AddStaminaPoints((GetHealRate() * GetMaxStaminaPoints())/100);  
          } else {
...
mixed limbs) {
    ...
    if( !limbs ) {
        if( internal ) {
            ...
            /*
            if (GetRecoveryTime() < 2) AddRecoveryTime(1);
            */
            return x;
        }
        else {
    ...
    if( internal ) {
        ...
        if (type == PIERCE || type == SLASH && !GetUndead()) {
                if (y >= 25)
                eventPrint("One of your cuts starts to bleed!");
        	AddBleeding(y/25);
        	}
        if (GetRecoveryTime() < 2) AddRecoveryTime(1);
        return y;
...
int eventCheckProtection(object agent, int type, int damage) {
    ...
    while(i--) {
        ...
        for(int tempcnt=1; tempcnt<MAX_DAMAGE_BIT; tempcnt++) {
          if((1 << tempcnt) & type)
            numdamtypes++;
        }
        for(int tempcntb=1; tempcntb<MAX_DAMAGE_BIT; tempcntb++) {
          if((1 << tempcntb) & type & Protection[i]->bits)
            shieldedcount++;
        }
