...
inherit LIB_DAEMON;
inherit LIB_HELP;
private int          AreaSpell = 0, GlobalSpell = 0;
...
varargs int CanCast(object who, int level, string limb, object array targets) {
  ...
  if (RecoveryTime) {
	...
	for(i=0; i<maxi; i++) {
	    ...
	    // Used to be if( max_hp - hp < (Healing[0]+Healing[1])/10 + 1 ),
	    // but that didn't seem to make sense and some players would not
	    // get healing they needed. Refer to /log/bugs/fixed/332_healing
	    if(hp >= max_hp) {
		if( limb ) {
