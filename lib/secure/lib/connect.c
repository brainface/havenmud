...
static void UserValidatePassword(string password, string firstpass, string cap);
static void UserEnterPassword(string password, string cap); 
...
/*  receive("\nAn email has been sent to " + email + " with a new password \n"
  ...
  receive("\n\nPlease enter a password (6 character minimum):");
  input_to( (: UserEnterPassword :), I_NOESC | I_NOECHO, cap);
}
...
static void UserEnterPassword(string password, string cap) {
	if(strlen(password) < 6) {
		...
		receive("\nAgain: ");
		input_to( (: UserValidatePassword :), I_NOESC | I_NOECHO, password, cap);
    }
}
static void UserValidatePassword(string password, string firstpass, string cap) {
	if(firstpass != password) {
	  ...
	  Player->SetPassword(crypt(password, 0));
	  Player->SetConfirmedEmail(1);
	  Player->SetNewServer(1);
	  CHARACTER_D->eventLink(Player->GetEmail(), Player->GetKeyName());
		/* ONWARD! */
...
// we no longer send an email so commenting out - Mel 2013-12-21
  eventEnterGame();
}
