...
int AddLanguagePoints(string lang, int points) {
	...
	Languages[key]["level"]++;
	if (Languages[key]["level"] >= 100) {
		 Languages[key]["level"] = 100;
		 eventPrint("You are now fully knowledged in " + key);
		 }
    }
...
int GetNextLevel(string lang, int curr_level) {
    ...
    if( !Languages[lang] ) return 0;
    y = 1 + (GetStatLevel("intelligence") / 40);
    y = (2000) / y;
    return y;
}
...
string SetBaseLanguage(string l) { 
  RemoveLanguage(GetNativeLanguage());
  SetLanguage(l, 100, 1);
