...
int GetValue(object ob, object who) {
    ...
    who->eventTrainSkill("bargaining", who->GetSkillLevel("bargaining"), GetSkillLevel("bargaining"), 1);
...
mixed CanBuy(object who, object *obs) {
    if( !present(StorageRoom, environment()) ) {
...
int eventBrowse(object who, string args) {
    ...
    if( !(sroom = present(StorageRoom, environment())) ) {
...
mixed eventBuy(object who, object array obs) {
    ...
    sroom = present(StorageRoom, environment());
    if (!sroom) {
      eventForce("speak I'm unable to help you right now.  Check back later.");
      return 1;
    }
	...
	if( !ob->CanSell(who) ) {
        ...
        who->eventPrint(capitalize(GetKeyName()) + " hands you " +
...
mixed eventSell(object who, string what) {
    ...
    eventForce("speak here is " + (string)ob->GetShort() + " for " + cost +
               " " + GetLocalCurrency() + "!");
    eventForce("give first " + what + " to " + (string)who->GetKeyName());
