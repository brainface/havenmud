...
static void create() {
  ...
  SetInventory( ([ 
    HAVENWOOD_AREAS + "darkorderguild/guard" : 1,
  ...
  AddEnter("shack", HAVENWOOD_AREAS + "darkorderguild/main", (:AcceptableCheck:));
}
...
int AcceptableCheck() {
  ...
  if((guard) && (living(guard))) {
    if(who->GetGuild() != "darkorder") {
      message("system", "The guard blocks you from entering.", this_player());
