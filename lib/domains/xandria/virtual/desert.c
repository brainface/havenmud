...
void AddSpawns(int x, int y) {
  ...
  if ((x < -20 && x > -100) || (x > 20 && x < 100)) {
    ...
    if (!random(10)) AddInventory(XANDRIA_VIRTUAL "/desert/scorpion",random(3)+1);
    // slave hunter
    // escaped slave
  }
  ...
  if ((x <= -100 && x > -250) || (x >= 100 && x < 250)) {
    SetShort("deep in the sands of the Xandrian Desert");
  ...
  // deep desert (avatar+)
  if ( x <= -250 || x >= 250 ) {
    SetShort("lost in the sands of the Xandrian Desert");
