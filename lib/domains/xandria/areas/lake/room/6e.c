...
static void create() {
  ...
  SetLong(
    ...
    "just on the edge of the desert. To the south stands impassable "
    "rapids, where no person has ever dared to cross and lived. "
  );
  SetItems( ([
    ({ "water", "waters", "lake" }) :
    ...
    ({ "rapids" }) : "The rapids are surely impassable.",
  ]) );
  SetItemAdjectives( ([
  "waters" : ({ "dark", "deep" }),
  "rapids" : ({ "impassable" }),
