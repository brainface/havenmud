...
static void create() {
   ...
   SetItems( ([
      ...
      ({ "walls", "wall" }) :
         ...
         "northern wall.",
      ({ "mirror" }) : 
      	  "As you gaze into the mirror you notice that you have "
      	  "no reflection in the mirror and that the room in the mirror is "
      	  "completely different from the one you are standing in.",
      ({ "floor" }) :
   ...
   SetExits( ([ 
      "northwest" : FCASTLE_ROOM "mirror7",
   ]) );
   SetInventory( ([
   ]) );
}
