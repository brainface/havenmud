/*                     
 * Runed obsidian scimitar  
 * deals additional damage based upon magic boost skill.
...
static void create() {
  ...
  SetRepairDifficulty(80);     SetProperty("magic",
    "This scimitar has been embued with the power"
...
int eventStrike(object who) {                
  ...
  if (!random(7) && GetProperty("hasMagicBoost") == 1) {
      amount = random(GetProperty("MagicBoostAmount")); 
      message("action",                                 
        environment()->GetCapName() + "'s scimitar " +       
...
int SetupProc() {                                                      
  ...
  // only half value, though.                                             
    ...
    MagicBoostAmount = wielder->GetSkill("practical magic")["level"] / 4;
  ...
  if (!hasMagicBoost || !MagicBoostAmount) {
    ...
    send_messages( ({"wield"}),
      "$agent_name $agent_verb an obsidian scimitar.",
  ...
  // wield message
  send_messages( ({"wield"}),
    "$agent_possessive_noun scimitar " +GetProperty("MagicBoostDesc") +
