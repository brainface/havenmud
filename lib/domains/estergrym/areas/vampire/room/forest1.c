...
static void create() {
  ...
  SetLong(
     "A path into a dark forest begins here, the trees "
     ...
     "to determine whether it is light out. You can make out an exit "
     "to the north road just east of here." 
     );
  SetSmell( ([
  ...
  SetItems( ([
     ({"path", "pathway", "trail"}) : "Surrounded by dense forest the small "
       "path wends its way to the northwest from here. Littered with dead "
       "leaves the ground seems strangely undisturbed. ",
  ...
  SetExits( ([
     "northwest" : VAMP_ROOM + "forest2",
     ]) );
  AddExit("east", "/domains/estergrym/virtual/northroad/1,25", (: PreExit :) );

}

int PreExit() {
  if (this_player()->id("vamp_wander")) {
