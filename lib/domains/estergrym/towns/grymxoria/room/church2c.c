...
static void create() {
  ...
  SetLong(
    "A large antechamber in the mother church of Aberach. "
    "The room is long and narrow leading from a large curving "
    "staircase down, to a equally large staircase that leads up "
    "to the basilica. A small door leads off to each side."
  );
  SetItems( ([
    ({ "stairs", "staircases", "down stairs", "up stairs" }) :
...
int PreExit(string dir) {
  if ((this_player()->GetReligion(1) == "Aberach") || (creatorp(this_player()))) {
    return 1;
  }
  else {
