...
*/
  ...
  SetLong(
     ...
     "high into the sky. A small canvas tent is seen directly to the north. "
  );
  SetItems( ([
     ...
     ({ "tent" }) : "A small canvas tent is seen directly to the north. ",
  ]) );
  SetItemAdjectives( ([
     "gate" : ({ "trading","post" }),
     "walls" : ({ "trading","post" }),
  ...
  SetExits( ([
     "south" : S_ROOM + "sulumus3",
     "southeast" : S_ROOM + "sulumus2",
     "east" : S_ROOM + "sulumus5",
  ]) );
}
