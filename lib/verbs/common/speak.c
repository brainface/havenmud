...
mixed can_speak_in_wrd(string str) {
   if(this_player()->GetSleeping()) {
    ...
    if( str ) return "What do you wish to speak?";
    else return 0;
}
...
mixed do_speak_str(string str) {
    ...
    if (defaultlang = this_player()->GetProperty("defaultlanguage"))
    	return do_speak_in_wrd_str(defaultlang, str);
    return do_speak_in_wrd_str(nativelang, str);
}
...
string GetHelp(string str) {
    return ("Syntax: <speak MESSAGE>\n"
	    ...
	    "See also: message, speak, shout, speak, tell");
}
