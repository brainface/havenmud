...
varargs mixed CanLock(object who, string id) {
    if( GetLocked() ) {
...
mixed eventLock(object who, object key) {
    string ids = key->GetKeyID();

    if( member_array(ids, GetKeys()) == -1 ) {
...
varargs mixed eventPick(object who, string id, object tool) {
    ...
    if( !tool ) {
	    strength = (int)who->GetSkillLevel("lockpicking");
    } else {
	    bonus = tool->GetPickBonus();
	    strength = who->GetSkillLevel("lockpicking") + bonus;
    }
    ...
    if( Pick ) {
	    ...
	    if( tmp != 1 ) {
	      if( !tmp ) {
		      ...
		      who->AddExperience(LockStrength/2);
		      return 1;
	      }
	    return tmp;
	    }
		  who->AddExperience(LockStrength*2);
	    if( SetLocked(0) ) { return 0; }
	    return 1;
    }
    if( strength > ( LockStrength / 10 + random(LockStrength) ) ) {
		  who->AddExperience(LockStrength*2);
	    if( SetLocked(0) ) { return 0; }
	    send_messages("pick", "$agent_name $agent_verb the lock on $target_name!", who, this_object(), environment(who));
	    return 1;
    }
   who->AddExperience(LockStrength/2);
   send_messages("fail", "$agent_name $agent_verb in $agent_possessive attempt to pick the lock on $target_name.",
		  who, this_object(), environment(who));
   if( random(100) > strength ) {
	    send_messages("cut", "$agent_name $agent_verb $agent_reflexive on the lock.", who, this_object(), environment(who));
      who->eventReceiveDamage(this_object(), 8, random(10) + 1,  who->GetWieldingLimbs()[random(sizeof(who->GetWieldingLimbs()))]  );
      }
   return 1;
}
