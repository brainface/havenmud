...
mixed can_scream() {
  ...
  if (who->GetConditionFlag(CONDITION_PREVENT_COMBAT)) return "Violence is beyond you at the moment.";
  if (who->GetConditionFlag(CONDITION_PREVENT_TALK)) return "Your voice is gone!";
  return 1;
}
...
int eventScream(object who, object target) {
  ...
  // mahk 2017:
  // half damage if they can't hear (I assume some physical damage is inherent)
  if (target->GetConditionFlag(CONDITION_PREVENT_HEAR)) level /= 2;
