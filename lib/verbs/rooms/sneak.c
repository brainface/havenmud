...
mixed do_sneak_wrd(string dir) {
  ...
  foreach(object thing in all_inventory(environment(this_player()))) {
    if (living(thing) && thing != this_player() && !creatorp(thing) )
      livs += thing->GetStatLevel("wisdom");
    if (thing->GetKeyName() == "guard object") {
      if (thing->GetBlocking() == dir && thing->GetOwner() != this_player()) {
        guard = thing;
      }
    }
  }
  ...
  if (guard && !this_player()->GetInvis()) {
    send_messages( ({"spot"}),
       "$target_name $target_verb $agent_possessive_noun feeble attempt to "
       "skulk past.",
         this_player(), guard->GetOwner(), environment(this_player()));
      return 1;
  }
