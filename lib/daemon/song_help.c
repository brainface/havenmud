...
static private void LoadIndices() {
  ...
  tmp_spells = ({ });
  foreach(string spell_type in get_dir(DIR_SONGS "/")) {
    if (spell_type == "test_songs") continue;
    if (spell_type == "include") continue;
    if (spell_type == "obj") continue;
   Indices[spell_type] = ({ });
   tmp_spells = get_dir(DIR_SONGS + "/" + spell_type + "/*.c");
   if (sizeof(tmp_spells)) {
      foreach(string file in tmp_spells) {
          file = DIR_SONGS + "/" + spell_type + "/" + file;
          Indices[spell_type] += ({ file->GetSong() });
        }
      }
   }
...
string GetHelpByIndex(string index, string topic) {
	...
	if( !(help = ob->GetHelp(topic)) ) {
       ...
       sscanf(base_name(ob), "/std/songs/%s/%s/", spell_dir, spell);
