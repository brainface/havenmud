...
void eventReceiveFingerRequest(mixed *packet) {
    mixed *ret;
    CHAT_D->eventSendChannel("Finger", "spam", "Received finger request for " + packet[6] + " from " +
                             packet[3] + "@" + packet[2] + ".");
    if( !(ret = (mixed *)FINGER_D->GetRemoteFinger(packet[6])) ) {
      INTERMUD_D->eventWrite(({ "error", 5, mud_name(), 0, packet[2], packet[3], "unk-user", capitalize(packet[6]) + " is not involved.", packet }));
	    return;
    }
