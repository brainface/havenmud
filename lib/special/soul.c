...
mapping SetMessageVerbs(mapping v) { return MessageVerbs = v; }

string *GetMessageVerbs(string rule) { 
	if (!MessageVerbs[rule]) return ({ Verb });
        if (!arrayp(MessageVerbs[rule])) {
          return ({ MessageVerbs[rule] });
          }
	return MessageVerbs[rule];
}
...
mapping SetMessages(mapping m) { return Messages = m; }

string GetMessage(string rule) { return Messages[rule]; }
