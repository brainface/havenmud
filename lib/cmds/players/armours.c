// made by duuk? mel added color/ limb order
// mahk 2018: added resistance readout
#include <lib.h>
...
string GetTotalProtection(object who, string limb);
...
mixed cmd(string args) {
  ...
  foreach(string local in limbs) {
    if(who->GetLimbClass(local) == 1 && sizeof(who->GetLimbChildren(local)) > 1) {
      torso = local + ":" + GetTotalProtection(who,local) + "\n"; //this is my main limb
      ...
      foreach(string limb in sort_array(who->GetLimbChildren(local), 1)) {
        if(regexp(limb, "^head$")) {
          head = limb + ":" + GetTotalProtection(who,limb) + "\n";
          if(sizeof(who->GetWorn(limb))) head += BuildPrint(head, limb, who, color, filter);
        }
        if(regexp(limb,  "arm$")) {
          if(arms) {
            arms += limb + ":" + GetTotalProtection(who,limb) + "\n";
          ...
          foreach(string subchild in who->GetLimbChildren(limb)) {
            arms += subchild + ":" + GetTotalProtection(who,subchild) + "\n";
            if(sizeof(who->GetWorn(subchild))) arms += BuildPrint(arms, subchild, who, color, filter);
          }
        }
        if(regexp(limb,  "leg$")) {
          if(legs) {
            legs += limb + ":" + GetTotalProtection(who,limb) + "\n";
          ...
          foreach(string subchild in who->GetLimbChildren(limb)) {
                 legs += subchild + ":"+GetTotalProtection(who,subchild)+"\n";
        ...
        if(!regexp(limb, "(leg$|arm$|^head$|^torso$)")) {
          if(!other) {
            other = limb + ":"+GetTotalProtection(who,limb)+"\n";
          ...
          foreach(string subchild in who->GetLimbChildren(limb)) { //this is probably unnecessary but
            other += subchild + ":"+GetTotalProtection(who,subchild)+"\n";   //added for good measure
...
// gets percentage damage negated from all armour on limb.
string GetTotalProtection(object who, string limb) {
  int protection = 0;
  foreach( object armour in who->GetWorn(limb)) {
    protection += armour->GetProtection(BLUNT|SLASH|PIERCE);
  }
