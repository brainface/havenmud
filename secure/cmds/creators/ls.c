...
static private string display_ls(mixed targ, int aflag, int lflag, int tflag, int nflag, int bflag, int sflag);
...
int cmd(string str) {
    ...
    if(sizeof(files = files - dirs))
      show = display_ls(file_mapping(files),all_files,long_details, time_sort, no_load_info, brief, size);
    else show = "";
    if(!(maxi = sizeof(dirs))) {
        if(moref) previous_object()->eventPage(explode(show, "\n"), "system");
...
static private string long_list(string dir, mixed *files) {
    ...
    for(i=0, maxi=sizeof(files); i<maxi; i++)
    {
        ...
        adder = sprintf("%:-3s%:-5s%:-30s%-10d%s",
          loaded, acc, ctime(files[i][2]),
          files[i][1], files[i][0]);
        if(files[i][1] == -2) adder = "%^BOLD%^YELLOW%^"+ adder + "/";
        else
        {
          switch(files[i][0][<2..<1])
          {
	    case ".o": adder = "%^RED%^BOLD%^" + adder; break;
            case ".h": adder = "%^GREEN%^" + adder; break;
            case ".c":
              adder = "%^CYAN%^" + adder;
              if(loaded == "*") adder = "%^BOLD%^" + adder;
              break;
          }
        }
        ret += adder + "%^RESET%^\n";
    ...
    for(i=0; i<rows; i++)
    {
        for(j=0; j<cols; j++)
        {
			...
			break;
                    case ".c":
                        pre = "%^CYAN%^";
                        if( trim(newfiles[ind])[0] == '*' )
                            pre += "%^BOLD%^";
                        break;
                }
                if(trim(newfiles[ind])[<1] == '/')
                    pre = "%^BOLD%^YELLOW%^";
                if(pre != "")
                    post = "%^RESET%^";
                ret += pre + sprintf("%:-"+(long+3)+"s", newfiles[ind]) + post;
            }
            else
                ret += sprintf("%:-"+(long+3)+"s", "");
        }
        ret += "\n";
    }
    return ret;
...
void help() {
    message("help", "Syntax: <ls [-ablmnst] (directories|files)>\n\n"
      ...
      "    -m Do not page the output through more\n"
      ...
      "    Size of the file\n"
      "    File name\n\n"
      "Ls also shows filenames in different colors depending on the type "
      "of file:\n"
      "    Unknown or strange endings are normal colored.\n"
      "    .c non-active files are %^CYAN%^cyan%^RESET%^.\n"
      "    .c active files are %^BOLD%^CYAN%^bold cyan%^RESET%^.\n"
      "    .h files are %^GREEN%^green%^RESET%^.\n"
      "    .o files are %^RED%^BOLD%^bold red%^RESET%^.\n"
      "    directories are %^BOLD%^YELLOW%^yellow%^RESET%^.\n\n"
