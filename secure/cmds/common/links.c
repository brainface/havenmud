...
mixed cmd(string args) {
  ...
  if (CHARACTER_D->CanAdvancedStart(who)) {
    this_player()->eventPrint("Registered with email " + who + " (Advanced Start Ok)");
    }
  else {
  this_player()->eventPrint("Registered with email " + who);
  }
  this_player()->eventPrint(sprintf("%-20s %-15s %-15s %-15s %-s", "Name", "Class", "Town", "Religion", "Level"));
  ...
  foreach(string person in characters) {
    ...
    if (!immortal_exists(person)) {
      this_player()->eventPrint(sprintf("%-20s %-15s %-15s %-15s %-d", capitalize(person), cl, Town, Religion[0], Level));
    } else {
