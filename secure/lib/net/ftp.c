...
/* Zaxan@Haven - This allows imms access to all files they would normally have in-game */
nomask static int check_privs(string file, string oper) {
	...
	{
	    eventWrite("550 Failure checking permissions.  Please report what you were doing "
                        "to the mud admin.\n",0);
	    return 0;
	}
...
private string GetFtpWelcomeMsg(){
    return sprintf("220- %s FTP server ready.\n%s"
		 "220 Please login with your creator name"+(ALLOW_ANON_FTP ? " or anonymous" : "")+".\n",
		 mud_name(),
...
void StartService(){
   ...
   for(port=MIN_PASV_PORT;port <= MAX_PASV_PORT;port++)
      if(member_array(port, FTP_PORT_TRACKER_D->QueryUsedPorts()) == -1) break;
...
private void eventCmdPswd(string arg){
    ...
    if(member_array(lower_case(Session->user), ({"anonymous", "ftp"})) != -1 && ALLOW_ANON_FTP){
...
private void eventCmdPort(string arg){
    ...
    Session->dataPipe = new(LIB_FTP_DATA_CONNECTION);
...
private void eventCmdRetr(string arg){
    ...
    log_file("inet/ftp_transfer",
	  sprintf("%s downloaded %s at %s.\n", Session->user, target_file, ctime(time())));
    switch(Session->binary){
        case 0:	
...
private void eventCmdStor(string arg){
    ...
    Session->filepos = 0;
    log_file("inet/ftp_transfer",
	  sprintf("%s uploaded %s at %s.\n", Session->user, absolute_path(arg), ctime(time())));
...
void eventRead(string data){
    ...
    if (!dispatchTo){
        log_file("inet/ftp_error",
                 sprintf("ftp - unknown command: %s at %s\n",cmd,ctime(time()))); 
