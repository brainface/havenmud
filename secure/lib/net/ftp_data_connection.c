...
varargs void eventWrite(mixed val, int retry) {
    int x;

    if (!Socket) return;
    if (!val || undefinedp(val)) return;

    if (!retry)
    {
        // This is a retry, don't clobber the buffer
        if( Socket->Buffer ) Socket->Buffer += ({ val });
        else Socket->Buffer = ({ val });
    }
    if( Socket->Blocking ) return;

    // Some FTP clients (e.g. SmartFTP) are icky and issue commands before we've accepted the passive connection.
    if (PassiveMode && !Socket->PassiveDescriptor)
    {
        // Re-call ourselves to try again. Indicate that this is a retry.
        call_out( (: eventWrite, val, 1 :), 1);
    }
    else
    {
        eventWriteCallback(PassiveMode ? Socket->PassiveDescriptor : Socket->Descriptor);
    }
}
