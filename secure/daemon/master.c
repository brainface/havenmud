...
int check_access(object ob, string fun, mixed file, string *ok, string oper) {
    ...
    if( Unguarded == ob ) {
        ...
        if( (tmp = base_name(ob)) ==  LIB_PLAYER || tmp == LIB_CREATOR ) {
            ...
            else if( file == DIR_PLAYERS+"/"+PlayerName[0..0]+"/"+PlayerName + 
                __SAVE_EXTENSION__)
              return 1;
            else if( file == DIR_CRES+"/"+PlayerName[0..0]+"/"+PlayerName +
                __SAVE_EXTENSION__)
              return 1;
            else i = sizeof(stack = ({ ob }));
        }
        else if( tmp == file  || tmp + __SAVE_EXTENSION__ == file ) return 1;
        else i = sizeof(stack = ({ ob }));
    }
    ...
    while(i--) {
        ...
        privs = explode(priv, ":");
        if(member_array(PRIV_ADMIN, privs) != -1) continue;
...
object compile_object(string str) {
    ...
    if(sscanf(str, DOMAINS_DIRS+"/%s/%*s", nom))
...
static void crash(string err, object who, object what) {
    write_file(DIR_LOGS "/shutdowns",
                mud_name() + " crashed " + ctime(time()) + " with error " + err +
                " from " + who->GetCapName() + " with " + file_name(what) + ".\n");
    message("system", "Reality implosion!!!  Everyone duck!!!", users());
    message("system", "You are being forced to quit.", users());
    users()->cmdQuit("master");
}
...
int valid_bind(object binder, object old_owner, object new_owner) {
    ...
    if( member_array(PRIV_SECURE, explode(query_privs(binder), ":")) != -1 )
      return 1;
  if( member_array(PRIV_ADMIN, explode(query_privs(binder), ":")) != -1 )
    return 1;
    return 0;
}
...
int valid_hide(object who) {
  ...
  if (member_array(PRIV_SECURE, explode(priv, ":")) != -1) return 1;
  else return (member_array(PRIV_ADMIN, explode(priv, ":")) != -1);
}
...
int valid_socket(object ob, string fun, mixed *info) {
    ...
    while(i--) {
        ...
        if( !sizeof(explode(tmp, ":") &
          ({ PRIV_SECURE, PRIV_MUDLIB, PRIV_CMDS, PRIV_GENERAL, PRIV_ADMIN })) ) return 0;
    }
    return 1;
}
...
string parser_error_message(int type, object ob, mixed arg, int flag) {
	...
	else err = "There are not that many of them.";
	break;
...
object player_object(string nom) {
    ...
    if( tmp != CMD_ENCRE && tmp != CMD_DECRE && tmp != LIB_CONNECT && tmp != LIB_FTP )
	return 0;
...
static void eventReset() {
  ...
  obs = objects();
  CHAT_D->eventSendChannel("RESET", "resets", "Reset Number " + ResetNumber + " started with " + sizeof(obs) + " objects.");
  y = 0;
  foreach(ob in obs) {
	  function f;
  	if( !ob ) {
  ...
  CHAT_D->eventSendChannel("RESET", "resets", "Reset Number " + ResetNumber +": " +  x + " objects reclaimed, " + (sizeof(obs) - y) + " objects reset, " + y + " objects cleaned.");
}
