...
static void SetupRoom(int x, int y) {
    ...
    // two signs:
    // (durg handled separately)
    if (x == 0 && y == 30)  inv += ([ FROSTMARCHES_VIRTUAL "signs/frostmarchrd030" : 1 ]);
    if (x == 0 && y == 49)  inv += ([ FROSTMARCHES_VIRTUAL "signs/frostmarchrd049" : 1 ]); 

    // some wanderers.
    if (!random(50)) inv += ([ STD_NPC "kobold" : 1 ]);
    SetLong(long);
    SetItems(items);
    SetInventory(inv);
