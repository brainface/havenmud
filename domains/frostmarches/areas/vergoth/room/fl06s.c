...
static void create() {
  room::create();
  SetShort("sixth floor stairs along the south wall");
  SetLong("The stairs continue up into a seventh floor.  They are very "
  ...
  SetItems( ([ ({"wall", "walls","floor"}) : "The walls and floor are "
	...
	"window" : "This small window is barely large enough to see Kailie.  "
		"Looking out the window though, the tops of the trees can be seen." ,
	]) );
  ...
  SetExits( ([ "northeast" : V_ROOM + "fl06e",
	       "northwest" : V_ROOM + "fl05w" ]) );
}