...
static void create() {
   room::create();
   SetShort("before a large door");
   SetClimate("indoors");
   SetProperty("lower plane",1);
   SetAmbientLight(35);
   SetLong(
     "The hallway comes to a dead end here infront of a large door, a small "
     "stairway leads back down to the third floor. The walls are pocked with "
     "a few scorch marks however two torches sit on the walls on either side "
     "of the door. The floor is covered in rather large burn marks "
   ...
   SetItems( ([
      ...
      ({ "walls", "wall" }) :
         ...
         "for two torches that line the large door to the north.",
      ({ "mark", "marks" }) :
      ...
      ({ "torch", "torches" }) :
         ...
         "provide no light, they rest on either side of the door. ",
      ({ "ash", "piles of ash" }) :
   ...
   SetExits( ([ 
      "north" : FCASTLE_ROOM "throne",
      "down" : FCASTLE_ROOM "room13",
   ]) );
   SetInventory( ([
   ]) );
   SetDoor("north", FCASTLE_OBJ "thronedoor");   
  call_other( FCASTLE_OBJ "thronedoor", "SetClosed", 1);
  call_other( FCASTLE_OBJ "thronedoor", "SetLocked", 1);
}
