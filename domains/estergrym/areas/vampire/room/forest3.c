...
static void create() {
  ...
  SetLong(
     "The path continues to the west and south, the trees "
     "only just making way. The ground here is littered with "
     "dead and brittle leaves much like the rest of the path "
     "but have been trampled in places. There are tracks of some "
     "unknown animal here. The trees are dense despite, for all "
  ...
  SetListen( ([
     "default" : "A faint howling or baying of some animal can be heard periodically.",
     ]) );
  SetItems( ([
     ({"path", "pathway", "trail"}) : "Surrounded by dense forest the small "
       "path wends its way to the west and south from here. Littered "
  ...
  SetExits( ([
     "south" : VAMP_ROOM + "forest2",
     "west" : VAMP_ROOM + "forest4",
     ]) );
