...
static void create() {
  ...
  SetLong(
     ...
     "the town hall stands. To the west the road leads into assassin "
     "lane, while the church stands directly to the south. "
  );
  SetItems( ([
     ({"town","cavern"}) : (:GetLong:),
  ...
  SetEnters( ([
     "church" : V_ROOM + "church/ch1",
  ]) );
  SetExits( ([
     "north" : V_ROOM + "main_road/mr7",
  ]) );
  }
