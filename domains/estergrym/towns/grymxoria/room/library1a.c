...
static void create() {
  ...
  SetItems( ([
    ...
    ({ "stair", "stairs", "stair case" }) : "A stone stair case going up." 
  ]) );
  SetInventory( ([
    G_NPC "karn" : 1,
    G_NPC "agnod" : 1,
...
int PreExit(string dir) {
  if((this_player()->GetReligion() == "Aberach") &&
    (this_player()->ClassMember("deathknight") ||
    this_player()->ClassMember("soulreaver") ||
    (this_player()->GetClass() == "necromancer") ||
    (creatorp(this_player())))) {
  return 1;
  } else write("A strong magical force bars your way.");
  return 0;
}
