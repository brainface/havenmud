...
static void eventDawn() {
    ...
    obs = filter(users(), (: environment($1) &&
                           ...
                           (string)environment($1)->GetClimate()!="underground" &&
                           !((int)environment($1)->GetProperty("no time")) :));
    message("environment", "%^YELLOW%^The sun appears just over the horizon.",
           obs );
    CHAT_D->eventSendChannel("", "time", "It is dawn. " + GetFormattedTime(), 1);
    i = sizeof(DawnCalls);
...
static void eventMorning() {
    ...
    message("environment", "%^BOLD%^YELLOW%^The sun now shines completely "
            "on a new day.", obs);
    CHAT_D->eventSendChannel("", "time", "It is morning. " + GetFormattedTime(), 1);
...
static void eventTwilight() {
    ...
    message("environment", "%^CYAN%^The sun begins to fall away into "
            "twilight.", obs);
    CHAT_D->eventSendChannel("", "time", "It is twilight. " + GetFormattedTime(), 1);
...
static void eventNight() {
    ...
    call_out( (: eventMidnight :), DAY_LENGTH * HOUR_LENGTH - GetCurrentTime() );
    TimeOfDay = "night";
    obs = filter(users(), (: environment($1) &&
                           (string)environment($1)->GetClimate()!="indoors" &&
                           (string)environment($1)->GetClimate()!="underground" &&
    ...
    CHAT_D->eventSendChannel("", "time", "It is night. " + GetFormattedTime(), 1);
    i = sizeof(NightCalls);
...
static void eventMidnight() {
    ...
    if( CurrentDay > i ) {
        ...
        eventConfigure();
        if( y != CurrentYear )
          CHAT_D->eventSendChannel("", "%^YELLOW%^Happy New Year!!! %^RESET%^ " +
                  "It is now the year " + GetYearString(CurrentYear) + "!!!!!", 1);
        return;
    }
    call_out( (: eventDawn :), Dawn);
    TimeOfDay = "night";
    CHAT_D->eventSendChannel("", "time", "It is midnight. " + GetFormattedTime(), 1);
...
string GetFormattedTime() {
	return sprintf("Time: %s", mtime(time()));
}
